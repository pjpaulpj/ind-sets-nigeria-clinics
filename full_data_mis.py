...
def create_distance_matrix(data, radius):
	...
	data['coords'] = data[['properties/latitude', 
			       'properties/longitude']].apply(tuple, axis=1)
...
given an edge-list representation as a pandas data-frame. '''
	...
	adj_matrix = DataFrame(np.where(distances > radius, 1,0), 
			       index = data.id, columns = data.id)
	return(adj_matrix)
